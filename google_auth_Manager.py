...
class AuthManager:
    ...
    def __init__(self, credential_path:str = None, email:str =None, scope:str= None, re_auth: bool=False):
    ...
    def get_token(self, email: str):
        ...
        return AuthStore(Session()).get_token(email)

    def save_token(self) -> None:
        "Save newly created token details for a user"

        AuthStore(Session()).save_token(self.email, self.credential_path, self.cred_details)
        
    def update_token(self):
        "Update credential details"
        AuthStore(Session()).update_token(self.email, self.credential_path, self.cred_details)
    ...
    def authenticate(self):
        ...
        if self.email is not None and self.credential_path is not None and not self.re_auth:
            raise ValueError("User already authenticated - Please reauthenticate if needed")
        ...
        if self.email is not None and self.credential_path is None and not self.re_auth:
            ...
            self.cred_details:Credentials = (
                Credentials
                .from_authorized_user_info(
                    json.loads(token_details.token_data), 
                    self.scope
                )
            )
            if(
        ...
        if self.credential_path is not None and not os.path.exists(self.credential_path):
            raise FileNotFoundError(f"Credentials file not found: {self.credential_path}")
